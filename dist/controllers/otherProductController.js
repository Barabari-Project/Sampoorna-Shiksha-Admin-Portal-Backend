...
export const getOtherProductsByOrderId = expressAsyncHandler(async (req, res) => {
    const { orderId } = req.params;
    console.log(orderId);
    const otherProducts = await OtherProductModel.find({ order: orderId });
    res.status(200).json({ otherProducts });
});
export const addOtherProductByOrderId = expressAsyncHandler(async (req, res) => {
...
export const deleteOtherProductsById = expressAsyncHandler(async (req, res) => {
    const { id } = req.params;
    console.log(id);
...
//# sourceMappingURL=otherProductController.js.map