...
export const assignStockQuantity = expressAsyncHandler(async (req, res) => {
...
export const addNewStock = expressAsyncHandler(async (req, res) => {
    ...
    try {
        const updateList = toys.map((item) => {
            const { toy, quantity } = item;
            return StockModel.findOneAndUpdate({ toy: toy }, // Search condition
        ...
        const updatedOrder = await VendorOrderModel.findByIdAndUpdate(orderId, { isAddedOrRemovedFromTheStock: true }, { new: true, runValidators: true } // Return the updated document
        );
...
export const removeFromStock = expressAsyncHandler(async (req, res) => {
...
export const checkAvailableStock = expressAsyncHandler(async (req, res) => {
...
export const getStock = expressAsyncHandler(async (req, res) => {
    const stock = await StockModel.find().populate('toy');
    res.status(200).json({ toys: stock });
});
export const deleteToyFromStock = expressAsyncHandler(async (req, res) => {
    const { id } = req.params;
    console.log(id);
