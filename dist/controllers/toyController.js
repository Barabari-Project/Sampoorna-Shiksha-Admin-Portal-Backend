...
export const addToy = expressAsyncHandler(async (req, res) => {
    const { toy } = req.body;
...
export const updateToy = expressAsyncHandler(async (req, res) => {
    const { toy } = req.body;
    const toyId = toy.id;
...
export const getToys = expressAsyncHandler(async (req, res) => {
    const toys = await ToyModel.find();
    res.status(200).json({ toys });
});
export const getToyById = expressAsyncHandler(async (req, res) => {
...
export const deleteToyById = expressAsyncHandler(async (req, res) => {
    const { id } = req.params; // Extract the toy ID from the request parameters
    checkMogooseId(id, 'Toy');
...
//# sourceMappingURL=toyController.js.map