...
import { VendorOrderStatus } from '../utils/types.js';
...
export const writeDataToTheSheet = async (cart, from, to, schoolName) => {
...
export const removeQuantityAfterPlacingOrder = async (orderList) => {
...
export const placeOrder = expressAsyncHandler(async (req, res) => {
    const { cart, from, to, schoolId } = req.body;
    if (from == to) {
...
export const updateOrderById = expressAsyncHandler(async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        delete order.__v;
        const updatedOrder = await VendorOrderModel.findByIdAndUpdate(id, { $set: order }, { new: true, runValidators: true } // Return the updated document and run validators
...
export const getOrdersBySchoolId = expressAsyncHandler(async (req, res) => {
